...
def word_vec_split(input_str):
    ...
    for i in range(length):
        if i == 0:
            try:
                result = model.word_vec(input_str)
        ...
        tail = input_str[i:]
        try:
            head_vec = model.word_vec(head)
            tail_vec = model.word_vec(tail)
...
if __name__ == '__main__':
    a = ['user', 'duck1123', 'default', 'domain', 'derived', 'character', 'derived', 'from']
    b = ['relative', 'creator', 'creator']
