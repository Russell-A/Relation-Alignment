                    ...
                    if node2.name == self.tail:
                        for s1,edge1,o1 in node1.edges[node_start]:
                            for s2,edge2,o2 in node2.edges[node1]:
                                if (s1, o1) == (s2, o2):
                                    break
                                relation = ((edge1, 1), (edge2, 2))
                                path = (((edge1, 1), (edge2, 2)),'path')
                                self.paths.append(relation)
                                    ...
                                    for s2,edge2,o2 in node2.edges[node1]:
                                        for s3,edge3,o3 in node3.edges[node2]:
                                            if (s1,  o1) == (s2,o2) or (s1,  o1) == (s3,o3) or (s2,o2) == (s3,o3):
                                                break
                                            relation = ((edge1, 1), (edge2, 2),(edge3, 3))
                                            path = (((edge1, 1), (edge2, 2),(edge3, 3)),'path')
                                            self.paths.append(relation)
                                            line1 = '\t'.join((s1,edge1,o1))
                                            line2 = '\t'.join((s2,edge2,o2))
    ...
    def delete_unrelated_relations(self, candidate_path, num):  # 处理好path_relation set 和 counter dict 就好 todo:没测试过
        ...
        for item in self.path_realtion_set:
            for relation, no in item[0]:
                if relation == delete_relation and item[0] in self.dict_relation_structure:
                    # print(item[0])
            ...
            if not (relations in s.relation_path_set):
                ...
                for i in range(len(self.wait_for_complete)):
                    ...
                    if relation1 != relation2:
                        ...
                        s.counter_relation[new_relation] = s.counter_relation[relation1]*s.counter_relation[relation2]
                        l = tuple([tuple1+tuple2 for tuple1 in s.dict_relation_structure[relation1] for tuple2 in s.dict_relation_structure[relation2]])
                        s.dict_relation_structure[new_relation] = l
                        if new_relation in s.dict_relation_structure:
                            s.dict_relation_structure[new_relation] += l
                        else:
                            s.dict_relation_structure = l
                        if new_relation in self.dict_relation_structure:
                            self.dict_relation_structure[new_relation] += l
                        else:
                            self.dict_relation_structure = l
                        if not ( new_relation in self.dict_relation_node_degree):
                            self.dict_relation_node_degree[new_relation] = \
                                s.dict_relation_degree[relation1] * s.counter_relation[relation2]+ \
                                s.dict_relation_degree[relation2] * s.counter_relation[relation1]
                        else:
                            self.dict_relation_node_degree[new_relation] += \
                                s.dict_relation_degree[relation1] * s.counter_relation[relation2]+ \
                                s.dict_relation_degree[relation2] * s.counter_relation[relation1]
                        if not ( (new_relation,'multipath') in self.Counter_relation):
                            self.Counter_relation[(new_relation,'multipath')] = sqrt(\
                                s.counter_relation[relation1] * s.counter_relation[relation2]+ \
                                s.counter_relation[relation2] * s.counter_relation[relation1])
                        else:
                            self.Counter_relation[(new_relation,'multipath')] += sqrt(\
                                s.counter_relation[relation1] * s.counter_relation[relation2]+ \
                                s.counter_relation[relation2] * s.counter_relation[relation1])
...
if __name__ == '__main__':
    file = 'tv.tv_program.soundtrack.txt'
    for item in os.walk('./candidate1000/'):
        files = item[2]
    import time
    time1 = time.time()
    for file in files:
        print(file)
        s = structure_list('./candidate1000/'+file)
        s.shuffle()
